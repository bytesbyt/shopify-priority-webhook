...
function buildPriorityPayload(order) {
  ...
  return {
    ...
    CUST: process.env.PRIORITY_CUST ? Number(process.env.PRIORITY_CUST) : null,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!priorityUrl || !priorityUser || !priorityPass) {
      return res.status(500).json({ error: "Missing Priority credentials" });
    }

    const response = await axios.post(priorityUrl, payload, {
      auth: {
